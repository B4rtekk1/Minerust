...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
 ...
 let n=oct_decode(textureLoad(normal_tex,ip,0).xy); let v=normalize(u.camera_pos-origin); let bn=textureLoad(blue_noise,vec2<i32>(p&127u),0).xy; let r=fract(bn+hash2(p,u.frame_index)); let dir=sample_ggx(n,v,m.x,r);
